...
public:
	...
	{
		// NOTE(Wojciech): Here we do stuff because of the event pool stuff

		std::cout << "onUserActiveAudioChange() happened!" << std::endl;

		auto controller = SDKInterfaceWrap::GetInst().GetMeetingParticipantsController();
		...
		for (int i = 0; i < participants->GetCount(); ++i)
		{
			...
			ZOOM_SDK_NAMESPACE::IUserInfo* user = controller->GetUserByUserID(id);

			const wchar_t *name = user->GetUserNameW();
